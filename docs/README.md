...
![Docs map: Foundations (Reverse Proxy, OAuth/OIDC, JWT, Access Logging) feed into the Backend-specific deep dives (Postgres RLS, SSH Certificates), which feed into the Synthesis doc (Identity Bridging); suggested reading order runs left-to-right](../assets/README.svg)
